...
class Day {
public:
    string name, code, rm, courseName, programme;
    string section;
    int semester;
    Day() {}
    Day(string name, string code, string rm, int semester, string courseName, string section,string programme) : name(name), code(code), rm(rm), semester(semester), courseName(courseName), section(section), programme(programme) {}
    Day(string name, string code, string rm, int semester, string courseName) : name(name), code(code), rm(rm), semester(semester), courseName(courseName) {}
                                    ...
                                    if(i.section != k.section && i.instructorName != k.instructorName) {
                                        for(auto& d : room) {
                                            if(k.students <= d.first.second && !d.second && !k.isRepeated) {
                                                ...
                                                generateInstructorTimeTable(k.instructorName,k.courseCode,d.first.first,day,time);
                                                break;
                                            }
                                        }
                                    }
...
void printTimeTable(vector<vector<Day>>& v, string day) {
    ...
    for(auto& i : v) {
        ...
        else if(slot == 5) cout << "Slot M6 : 14 to 14:50\n\n";
        else if(slot == 6) cout << "Slot M7 : 15 to 15:50\n\n";

        for(auto& j : i) {
            cout << j.code << " | " << j.name << " | " << j.rm << " | Sem : " << j.semester << " | " << j.courseName << " | " << j.section << " | " << j.programme << '\n'; 
        } cout << "---------------------------------------------------------------\n\n";
        slot++;
    }
}
...
void printFreeSlot(list<Day>& extra, string day) {
    ...
    if(!extra.empty()) {
        ...
        cout << extra.back().code << " | " << extra.back().name << " | " << extra.back().rm << " | " << "Sem : " << extra.back().semester << " | " << extra.back().courseName << " | " << extra.back().section << " | " << extra.back().programme << endl;
        extra.pop_back();
    }

    if(!extra.empty()) {
        cout << "---------------------------------------------------------------\n\n";
        cout << "Slot M9 : 17 to 17:50\n\n";
        cout << extra.back().code << " | " << extra.back().name << " | " << extra.back().rm << " | " << "Sem : " << extra.back().semester << " | " << extra.back().courseName << " | " << extra.back().section << " | " << extra.back().programme << endl;
        extra.pop_back();
    }
...
/* ------------------------------------------------------------------------------------------------------------------------------- */
    ...
    //int cnt = 0;
    // for(auto& it : btech.year1) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl; 
    // for(auto& it : btech.year2) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;  
    // for(auto& it : btech.year3) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;  
    // for(auto& it : btech.year4) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;   
    // for(auto& it : btech.MnCy1) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;  
    // for(auto& it : btech.MnCy2) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;  
    // for(auto& it : btech.MnCy3) if(it.lectureHours != 0) cout << it.courseName << " " << it.lectureHours << "Sem : " << it.semester << endl;  
    //cout << cnt << endl;
